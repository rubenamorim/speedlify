...
function hasUrl(urls, requestedUrl) {
  ...
  if (requestedUrl && typeof requestedUrl === "string") {
    ...
    if (
      lowercaseUrls.indexOf(requestedUrl) > -1 ||
      (requestedUrl.endsWith("/") &&
        lowercaseUrls.indexOf(requestedUrl.substr(0, requestedUrl.length - 1)) >
          -1)
    ) {
      return true;
    }
...
function getLighthouseTotal(entry) {
  return (
    entry.lighthouse.performance * 100 +
...
module.exports = function (eleventyConfig) {
  ...
  eleventyConfig.addFilter(
    "getSites",
    (results, sites, vertical, skipKeys = []) => {
      let urls = sites[vertical].urls;
      let isIsolated =
        sites[vertical].options && sites[vertical].options.isolated === true;
